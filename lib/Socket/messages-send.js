...
const makeMessagesSocket = (config) => {
    ...
    const relayMessage = async (jid, message, {
        ...
        const decodedJid = WABinary_1.jidDecode(jid);

        if (!decodedJid) {
            logger.error({ jid }, 'Invalid JID received');
            return;
        }
        ...
        if (participant) {
            ...
            const participantDecoded = WABinary_1.jidDecode(participant.jid);
            if (!participantDecoded) {
                logger.error({ jid: participant.jid }, 'Invalid participant JID');
                return;
            }
        ...
        await authState.keys.transaction(async () => {
            ...
            if (isGroup || isStatus) {
                ...
                const meDecoded = WABinary_1.jidDecode(meId);
                if (!meDecoded) {
                    throw new Error(`Invalid meId: ${meId}`);
                }
                const { user: meUser } = meDecoded;
                ...
                for (const { user, device } of devices) {
                    ...
                    const jid = WABinary_1.jidEncode(
        ...
        const messageJSON = {
            ...
            message: message,
            messageTimestamp: (0, Utils_1.unixTimestampSeconds)(new Date()),
        ...
        sendMessage: async (jid, content, options = {}) => {
            ...
            if (typeof content === 'object' && 'disappearingMessagesInChat' in content &&
                ...
                if (messageType) {
                    switch (messageType) {
                        case 'PAYMENT':
                            ...
                            return await relayMessage(jid, paymentContent, { messageId: (0, Utils_1.generateMessageID)(), ...getParticipantAttr() });
                        case 'PRODUCT':
                ...
                if (isDeleteMsg) {
                    ...
                    additionalNodes.push({ attrs: { biz_bot: '1' }, tag: "bot" });
                }
